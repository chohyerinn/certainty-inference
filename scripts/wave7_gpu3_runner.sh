...
GPU=3
LOG_DIR="$ROOT/logs"
LOG="$LOG_DIR/wave7_gpu3_$(date +%Y%m%d_%H%M%S).log"

mkdir -p "$LOG_DIR" "$ROOT/reports" "$ROOT/submissions"
ln -sfn "$(basename "$LOG")" "$LOG_DIR/wave7_gpu3_latest.log"
cd "$ROOT" || exit 1
exec > >(tee -a "$LOG") 2>&1

echo "=== wave7 GPU3 runner start $(date -Is) ==="
...
run_reg mdeberta-v3-mnli-xnli-gpu3-cls-seed42 MoritzLaurer/mDeBERTa-v3-base-mnli-xnli 42 16 2e-5 10 fp16 --pooling cls
run_reg mdeberta-v3-mnli-xnli-gpu3-mean-seed42 MoritzLaurer/mDeBERTa-v3-base-mnli-xnli 42 16 2e-5 10 fp16 --pooling mean
run_reg mdeberta-v3-mnli-xnli-gpu3-clsmean-seed42 MoritzLaurer/mDeBERTa-v3-base-mnli-xnli 42 16 2e-5 10 fp16 --pooling cls_mean
run_reg mdeberta-v3-mnli-xnli-gpu3-mean-seed123-lr1e5 MoritzLaurer/mDeBERTa-v3-base-mnli-xnli 123 16 1e-5 12 fp16 --pooling mean

run_reg cross-encoder-nli-deberta-v3-base-gpu3-cls-seed42 cross-encoder/nli-deberta-v3-base 42 16 2e-5 10 fp16 --pooling cls
run_reg cross-encoder-nli-deberta-v3-base-gpu3-mean-seed42 cross-encoder/nli-deberta-v3-base 42 16 2e-5 10 fp16 --pooling mean
...
refresh_ensembles
echo "=== wave7 GPU3 runner end $(date -Is) ==="
