...
GPU=3
LOG_DIR="$ROOT/logs"
LOG="$LOG_DIR/wave5_gpu3_$(date +%Y%m%d_%H%M%S).log"

mkdir -p "$LOG_DIR" "$ROOT/reports" "$ROOT/submissions"
ln -sfn "$(basename "$LOG")" "$LOG_DIR/wave5_gpu3_latest.log"
cd "$ROOT" || exit 1
exec > >(tee -a "$LOG") 2>&1

echo "=== wave5 GPU3 runner start $(date -Is) ==="
...
run_reg klue-bert-base-gpu3-mean-seed42 klue/bert-base 42 16 2e-5 10 fp16 --pooling mean
run_reg kcbert-large-gpu3-mean-seed42 beomi/kcbert-large 42 8 1e-5 10 fp16 --pooling mean

run_reg ko-sroberta-multitask-gpu3-mean-seed42 jhgan/ko-sroberta-multitask 42 16 2e-5 10 fp16 --pooling mean
run_reg multilingual-e5-base-gpu3-mean-seed42 intfloat/multilingual-e5-base 42 16 2e-5 10 fp16 --pooling mean
run_reg multilingual-e5-base-gpu3-clsmean-seed42 intfloat/multilingual-e5-base 42 16 2e-5 10 fp16 --pooling cls_mean
run_reg multilingual-e5-large-gpu3-mean-seed42 intfloat/multilingual-e5-large 42 8 1e-5 8 fp16 --pooling mean

refresh_ensembles
echo "=== wave5 GPU3 runner end $(date -Is) ==="
